(function () {
    ...
    function onCheckboxChange(event){
        ...
        const imageValue = checkbox.value;

        if(checkbox.checked){
            deleteImages.add(imageValue);
        }else{
            deleteImages.delete(imageValue);
        }
    ...
    function updateDeleteImagesInput() {
        let deleteImagesInput = document.querySelector('input[name="deleteImagesInput"]');
        if (!deleteImagesInput) {
            deleteImagesInput = document.createElement('input');
            deleteImagesInput.setAttribute('type', 'hidden');
            deleteImagesInput.setAttribute('name', 'deleteImagesInput');
            form.appendChild(deleteImagesInput);
        }
        deleteImagesInput.value = Array.from(deleteImages).join(',');
    }

    function checkValidation(event){
        const nameError = nameErrorSpan.style.display === 'block';
        const imageError = imageInputSpan.style.display === 'block';
