...
const insertUser = async (req, res) => {
  try {
    ...
    let sendVerifyMailRes = await sendMail(email, otp);
...
const resendOtp = async (req, res) => {
  try {
    ...
    if (data) {
      let sendVerifyMailRes = await sendMail(
        req.session.data.email,
        req.session.data.otp
      );
      if (sendVerifyMailRes) {
...
const checkEmail = async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      req.session.otp = otp;
      if (user) {
        sendVerifyMailRes = otpService.sendVerifyMail(
          user.name,
          user.email,
          otp
        );
      ...
      if (user) {
        ...
        req.session.destroy((err) => {
          if (err) {
            ...
            return res
              .status(500)
              .json({ success: false, message: "Error destrying session" });
          }
...
const verifyLogin = async (req, res) => {
  try {
    ...
    if (!userData) {
      ...
      res.render("login", {
        message: "No users found with the email. please re-enter email",
        breadcrumb: "Log in",
        header: false,
        smallHeader: true,
        footer: false,
      ...
      res.render("login", {
        ...
        smallHeader: true,
        footer: false,
        message: "You are blocked. please contact our customer service.",
      });
    } else {
  ...
  try {
    ...
    const streamlinedProducts = products.map((product) => {
      ...
      if (finalOffer > 0) {
        ...
        offerApplied =
          finalOffer === productOffer ? "Product Offer" : "Category Offer";
      }
